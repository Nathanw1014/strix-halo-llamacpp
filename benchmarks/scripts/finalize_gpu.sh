...
env GGML_VK_MMID_ROWLISTS=1 timeout 900 "$ROWBIN/bin/llama-bench" -m "$Q35" -ngl 99 -fa 1 -b 512 -ub 512 -p 512 -n 0 -d 0,16384 -r 3 -o md > "$OUT/rowlists_on.md" 2>"$OUT/rowlists_on.err" && log "  on ok" || log "  on FAIL"
env GGML_VK_MMID_ROWLISTS=0 timeout 900 "$ROWBIN/bin/llama-bench" -m "$Q35" -ngl 99 -fa 1 -b 512 -ub 512 -p 512 -n 0 -d 0,16384 -r 3 -o md > "$OUT/rowlists_off.md" 2>"$OUT/rowlists_off.err" && log "  off ok" || log "  off FAIL"
sleep 15
